...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribeFraude = onSnapshot(
      collection(db, "fraudes"),
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        const unsubscribeSeance = onSnapshot(collection(db, "seance"), (snap) => {
          ...
          if (data.length > 0 && !selectedSeance) {
            setSelectedSeance(data[0].id);
          }

        });
        return () => unsubscribeSeance();
      } catch (err) {
